...
<div itemscope itemtype="https://schema.org/Blog" class="pshowblog row">
	...
	{if $blog}
    	{if !isset($isHook) || !$isHook}
    		...
    		{if Configuration::get('PSHOW_BLOG_SEARCH_ENGINE')}
    		<div class="col-xs-12 main-row-blog clearfix" id="search-blog" style="padding: 0px 0px 10px;">
    			<div class="col-xs-12 col-sm-4 col-md-3 title-of-search-blog">{l s='Search the blog' mod='pshowblog'}</div>
    			<form action="{PShowBlog::getSearchLink()}" method="post" class="col-xs-12 col-sm-8 col-md-9 search-items-form-box" enctype="multipart/form-data">
    				<input class="search_query form-control ac_input" id="search_query_item_blog" name="search_query_item_blog" placeholder='{l s="Szukaj" mod="pshowblog"}' value="" type="text">
    				<button type="submit" name="submitSearchBlog" id="submitSearchBlog" class="btn btn-default button-search">
    					<span>{l s="Szukaj" mod="pshowblog"}</span>
    				</button>
    			</form>
    		</div>
    		{/if}
    		{if count($sorting_options) > 0}
    			<div id="pshowblog_sort_bar" class="clearfix">
    				<form action="{PShowBlog::getCategoryLink($id_category)}" class="pshowblog_sort pull-right">
    					<label for="pshowblog_sort">{l s='Sort by:' mod='pshowblog'}</label>
    					<select id="pshowblog_sort" name="sort" class="">
    						{foreach $sorting_options as $sorting_option}
    							<option value="{$sorting_option['value']}" {if $sort eq $sorting_option['value']}selected="selected"{/if}>{if $sorting_option['value'] eq 'date_add:desc'}{l s='Date: newest' mod='pshowblog'}{elseif $sorting_option['value'] eq 'date_add:asc'}{l s='Date: oldest' mod='pshowblog'}{elseif $sorting_option['value'] eq 'views:desc'}{l s='Most viewed' mod='pshowblog'}{else}{l s='Most commented' mod='pshowblog'}{/if}</option>
    						{/foreach}
    					</select>
    				</form>
    			</div>
		...
		{else}
    		{if $title}
                <h1 itemprop="name" class="page-heading">{$title}</h1>
            {/if}
		{/if}
