	...
	{if $blog}
    	...
    	<div class="row row-eq-height layout_grid">
        	{foreach from=$blog item='entry'}
        		{if $entry@index != 0}
    				{if $entry@index == 1}<div class="col-xs-12 col-sm-12 col-md-6">{/if}
    					<article itemprop="blogPost" itemscope itemtype="https://schema.org/BlogPosting" style="{if $entry@index == 1}padding-top: 15px;{/if}{if $entry@index == 4}padding-bottom: 15px;{else}padding-bottom: 30px;{/if}border-bottom: 1px solid #d6d4d4;">
                                            ...
                                            <div class="row row-heading-entries-list row-eq-height" style="border: none;">
                                            	<div class="col-xs-12 row-heading-entries-list-info" style="text-align: left;">
                                                    {if $entry['date_add'] && $list_display_public_date}
                        								<small class="entry_date"><i class="material-icons">calendar</i>&nbsp;{$entry['date_add']}</small>
                                                    {/if}
    				...
    				{if $entry@index == (count($blog)-1)}</div>{/if}
				{else}
